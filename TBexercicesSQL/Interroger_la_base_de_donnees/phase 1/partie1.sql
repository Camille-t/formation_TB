...
-- 1) afficher toutes les infos concernant les employés
SELECT * 
FROM employe;

 -- 2) afficher toutes les infos concernant les départements
SELECT *
FROM dept;

-- 3) afficher le nom, la date d'embauche, le numéro de supérieur, le numéri de département et le salaire de tous les employés
...
-- 5) afficher les différentes valeurs des titres des employés (sans doublons)
...
SELECT nom, UPPER(nom) AS nom_maj, LOWER(nom) AS nom_min
FROM employe
WHERE nom = 'Vrante';

-- 32) Afficher le nom et le nombre de caractères du nom des employés. 
SELECT nom, LENGTH(nom) AS nb_caracteres_nom
FROM employe;
